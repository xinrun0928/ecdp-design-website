...
-- ----------------------------
`,zt=B({__name:"DutyRecordList",setup(A){const{allData:E,filteredData:u,loading:g,pagination:p,pagedData:f,total:T,deleteItem:I,upsertItem:y}=Lt({initialData:st}),e={id:0,workSummary:"",importantMatters:"",eventType:"",other:""},{drawerVisible:s,isEdit:S,formData:b,openCreate:h,openEdit:v}=gt({defaultFormData:e}),M=[{prop:"date",label:"值班日期",width:140},{prop:"rule",label:"排班规则",minWidth:180,showOverflowTooltip:!0},{prop:"person",label:"值班人员",width:140},{prop:"fillTime",label:"填报时间",width:180},{type:"actions",label:"操作",slot:"actions"}],D=[{label:"编辑",icon:ot,handler:l=>t(l)},{label:"删除",icon:dt,divided:!0,handler:l=>d(l)}];function o(){h()}function t(l){v(l)}async function d(l){await H(l.date)&&(I(r=>r.id===l.id),m.success("删除成功"))}function i(){const l=M.filter(c=>c.label!=="操作").map(c=>c.label),r=u.value.map(c=>[c.date,c.rule,c.person,c.fillTime]);$("值班记录",l,r)}function R(l){S.value?(y({...E.value.find(r=>r.id===b.id),...l},"id"),m.success("编辑成功")):(y({id:Math.max(...E.value.map(r=>r.id),0)+1,date:Z("YYYY-MM-DD"),rule:"视频会议保障人员",person:Q(),fillTime:J(),...l},"id"),m.success("新增成功")),s.value=!1,p.pageNum=1}function w(){s.value=!1}return(l,r)=>{const c=F("table-structure-drawer"),U=k,O=j,X=W,z=F("panel-layout");return P(),Nt(Ot,null,[a(z,{title:"值班记录"},{"title-extra":n(()=>[a(c,{sql:[_(Ut),_(At),_(ft),_(It),_(ht)]},null,8,["sql"])]),actions:n(()=>[a(U,{"margin-left":10,onClick:o},{default:n(()=>[...r[3]||(r[3]=[C("新增",-1)])]),_:1}),a(U,{type:"info","margin-left":10,onClick:i},{default:n(()=>[...r[4]||(r[4]=[C("导出",-1)])]),_:1})]),default:n(()=>[a(X,{"page-num":_(p).pageNum,"onUpdate:pageNum":r[0]||(r[0]=L=>_(p).pageNum=L),"page-size":_(p).pageSize,"onUpdate:pageSize":r[1]||(r[1]=L=>_(p).pageSize=L),data:_(f),columns:M,total:_(T),loading:_(g),"row-key":"id","show-tool-bar":!1},{actions:n(({row:L})=>[a(O,{items:D,row:L},null,8,["row"])]),_:1},8,["page-num","page-size","data","total","loading"])]),_:1}),a(bt,{visible:_(s),"onUpdate:visible":r[2]||(r[2]=L=>pt(s)?s.value=L:null),"edit-data":_(S)?_(b):void 0,onConfirm:R,onCancel:w},null,8,["visible","edit-data"])],64)}}});export{zt as default};
