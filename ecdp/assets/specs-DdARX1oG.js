import{aI as l,bk as c,bf as u,aN as m,aF as g,ab as S,ad as T,ap as y,aa as E}from"./element-plus-6J5z1N_S.js";const R=`# 异常处理规范
...
| \`flood_\` | 防汛检查（fhis-web） | 防汛巡查、隐患排查、预警 | \`flood_task\`、\`flood_inspection\`、\`flood_warning\` |
...
│ 汛期公路巡查系统  │ DOWNLOAD 下载 │ FLOOD_*_TYPE   │                  │
...
| \`FLOOD_INSPECTION\` | \`flood\` | 汛期公路巡查系统 |
...
`,f={logging:l,"error-handling":E,"database-design":y,"api-design":T,metrics:S,signing:g,"inter-service":m,"message-queue":u,"system-architecture":c};function L(n){return f[n]||l}const p={文档版本:"version",编制日期:"date",适用版本:"applies",文档状态:"status"};function x(n){const a=n.split(`
`),s={};let e=0;for(e+=1;e<a.length;){const i=a[e].trim();if(!i){e++;continue}if(i.startsWith(">")){const r=i.match(/^>\s*\*\*([^*]+)\*\*\s*:?\s*(.*)$/);r&&p[r[1]]&&(s[p[r[1]]]=r[2].trim()),e++;continue}if(i==="---"||i.startsWith("--- ")){e++;continue}break}return{meta:s,body:a.slice(e).join(`
`).trimStart()}}function t(n,a,s,e,i){const{meta:r,body:d}=x(e);return{key:n,title:a,file:s,raw:e,meta:r,body:d,category:i}}const o=[t("api-design","接口设计规范","接口设计规范",b,"backend"),t("error-handling","异常处理规范","异常处理规范",R,"backend"),t("logging","日志规范","日志规范",_,"backend"),t("database-design","数据库设计规范","数据库设计规范",v,"backend"),t("inter-service","服务间通信规范","服务间通信规范",A,"backend"),t("message-queue","消息队列规范","消息队列规范",O,"backend"),t("metrics","数据度量规范","数据度量规范",I,"backend"),t("signing","数据签名设计规范","数据签名设计规范",M,"backend"),t("system-architecture","系统架构设计说明书","系统架构设计说明书",N,"architecture")],D=[{key:"backend",title:"后端开发规范",icon:l,items:o.filter(n=>n.category==="backend")},{key:"architecture",title:"微服务架构设计",icon:c,items:o.filter(n=>n.category==="architecture")}];function P(n){return o.find(a=>a.key===n)}export{L as a,o as b,P as g,D as s};
